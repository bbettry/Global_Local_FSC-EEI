...
def create_model(name, n_cls, dataset='miniImageNet', dropout=0.1):
    """create model by name"""
    if dataset == 'miniImageNet' or dataset == 'tieredImageNet' or dataset == 'CUB_200_2011' or dataset == 'Power_2023_80':
        if name.endswith('v2') or name.endswith('v3'):
            model = model_dict[name](num_classes=n_cls)
        elif(name.endswith("kd")):
            model = model_dict[name](num_classes=n_cls)
        elif(name.endswith("ssl")):
            ...
            model = model_dict[name](num_classes=n_cls)
        elif name.startswith('MobileNet13'):
            model = model_dict[name](num_classes=n_cls)
        else:
